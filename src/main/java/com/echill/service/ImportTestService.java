...
public class ImportTestService {
    ...
    private void validateRow(ToeicExcelRowDto row, int index) {
        if (row.getPart() == 0 || row.getPart() < 1 || row.getPart() > 7) {
        ...
        // Option D can be null for Part 2 sometimes, but usually TOEIC has 4 options. 
        // For Part 2, it's 3 options. Let's allow D to be null only for Part 2.
        if (row.getPart() != 2 && row.getOptionD() == null) {
